...
export const AdminForm: FC = (props) => {
            ...
            if(res.data === true){
                ...
                return
                        ...
                        AdminService.createAdmin({
                            firstName: firstName, lastName: lastName, emailAddress: email, password: password
                        }).then(async (res) =>
                            AuthenticateService.login(email, password).then((res) => {
                                history.push(location.pathname.split('/add-organizer')[0])
                            }))
            }
        })
    ...
    return (
        <Dialog open={true} className={styles.formDialog}>
            <IconButton component={Link} to={location.pathname.split('/add-organizer')[0]} className={styles.closeButton}><CloseIcon/></IconButton>
            <DialogTitle className={styles.formTitle}>Add organizer</DialogTitle>
            <DialogContent className={styles.formDialogContent}>
                ...
                <DialogActions className={styles.submitAction}>
                    <Button
                        ...
                        disabled={emailError!=="" || passwordError!=="" || password2Error!=="" || firstNameError!=="" || lastNameError!==""}

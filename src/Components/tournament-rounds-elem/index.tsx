...
export const TournamentRoundListElem: FC<{
    round: {
        id: number, tournament: string, number: number, date: string, completedRuns: number,
        numberOfRuns: number, pathToLogs: string
    }, rounds: {
        id: number, tournament: string, number: number, date: string, completedRuns: number,
        numberOfRuns: number, pathToLogs: string
    }[], index: number, tournamentId: number, reloadData: any
}> = (props) => {
    ...
    const downloadFile = () => {
        ...
        LogService.getLogs(props.round.id).then((res) => {
            const file = new Blob([res.data], {type: "text/plain"})
            element.href = URL.createObjectURL(file)
            element.download = "logs-" + props.round.date + ".log"
        ...
        if (Date.now() >= Date.parse(props.round.date)) {
                    ...
                    <div className={styles.card}>
                        ...
                        <Grid container direction={"row"} justify={"flex-start"} alignItems={"flex-start"}
                            ...
                            <Grid item xs={3} className={styles.alignItems}>
                                ...
                                <div
                                    className={styles.roundText + ' ' + styles.biggerNum}>{props.rounds.length - props.index}</div>
                            </Grid>
                            <Grid item xs={6} className={styles.alignItems}>
                                <div className={styles.date}>{moment(props.round.date).format("DD.MM.YYYY HH:mm")}</div>
                                <div className={styles.runs}>Completed
                                    runs: {finishedRuns}/{props.round.numberOfRuns}</div>
                            </Grid>
                            <Grid item xs={3} className={styles.alignItems}>
                                <Button variant={"contained"} color={"primary"} component={Link}
                                        disabled={finishedRuns !== props.round.numberOfRuns}
                                        onClick={downloadFile}>DOWNLOAD LOGS</Button>
                            </Grid>
                        </Grid>
                    </div>
                )
            ...
            return (
                <Router>
                    ...
                    <Route path={location.pathname + "/tournament-rounds/form"}><TournamentRoundForm date={""}
                                                                                                     numberOfRuns={0}
                                                                                                     tournamentId={props.tournamentId}
                                                                                                     data={props.rounds}
                                                                                                     reloadData={props.reloadData}/></Route>
                </Router>
            )
        }
    }
